<?php
if ( ! class_exists( 'lsvr_row_block' ) ) {
    class lsvr_row_block extends AQ_Block {
	...
	function __construct() {
		...
		$block_options = array(
			'name' => __( 'Row', 'lsvr-toolkit' ),
			'size' => 'span12',
            'resizable' => 0
		);

		//create the widget
		parent::__construct( 'lsvr_row_block', $block_options );
				...
				'<dl class="block-bar">',
					'<dt class="block-handle">',
						...
						'<span class="block-controls">',
							'<a class="block-edit" id="edit-'.$number.'" title="Edit Block" href="#block-settings-'.$number.'">' . __( 'Edit Block', 'lsvr-toolkit' ) . '</a>',
						'</span>',
					'</dt>',
				'</dl>',
	...
	function form($instance) {
		...
		__( 'ROW: Drag block items here. Every block item with width less than 100% should be placed inside ROW.', 'lsvr-toolkit' ),
		'</p>';
	...
	function after_form($instance) {
		...
		$block_saving_id = 'aq_blocks[aq_block_'.$number.']';
			...
			echo '<div class="block-control-actions cf"><a href="#" class="delete">' . __( 'Delete', 'lsvr-toolkit' ) . '</a></div>';
